...
{
    public class EagerDisconnectedEntryService : BaseServices.EagerDisconnectedService<EntryDTO, Entry>
    {
        public EagerDisconnectedEntryService()
        {
        }

        public EagerDisconnectedEntryService(IEagerDisconnectedRepository<Entry> repo) : base(repo)
        {
        }
        ...
        public override void Update(UserDTO caller, EntryDTO entity, Expression<Func<IUpdateConfiguration<Entry>, object>> graph)
        {
            NullCheck.ThrowArgumentNullEx(entity);
            entity.CheckPermission(caller);
